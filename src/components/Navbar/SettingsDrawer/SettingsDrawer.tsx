...
const TranslationSelectionBody = dynamic(
  () => import("./TranslationSelectionBody"),
  {
    ssr: false,
  }
);
...
const SettingsDrawer = () => {
  ...
  if (settingsView === SettingsView.Body)
    header = <div>{t("settings.title")}</div>;
  if (settingsView !== SettingsView.Body) {
    header = (
  ...
  return (
    ...
    >
      ...
      {isSettingsDrawerOpen && settingsView === SettingsView.Reciter && (
        <ReciterSelectionBody />
      )}
      {isSettingsDrawerOpen && settingsView === SettingsView.Tafsir && (
        <TafsirSelectionBody />
      )}
    </Drawer>
  );
